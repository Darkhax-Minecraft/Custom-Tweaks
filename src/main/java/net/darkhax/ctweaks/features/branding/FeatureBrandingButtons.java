...
public class FeatureBrandingButtons extends Feature {
    ...
    public void onGuiOpened (GuiScreenEvent.InitGuiEvent.Post event) {
        ...
        if (event.getGui() instanceof GuiMainMenu) {
            ...
            for (final String promoLink : this.promoLinks) {

                final String[] params = promoLink.split("@@");
                final int x = res.getScaledWidth() - index * 25;
                final GuiGraphicButton button = new GuiGraphicButton(params[1], params[0], index, x, 5 + index / this.buttonWidth);
